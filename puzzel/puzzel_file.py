...
import time
class Puzzel:
    def __init__(self, array,zeroindex=None):
        self.array = array
        self.col_row = len(array[0])
        self.zeroindex=zeroindex
        if self.zeroindex!=None:
            self.zeroindex=zeroindex
        else:
            self.zeroindex=self.get0Index(self.array)
            
# '123456780'
    @property
    def solved(self):
        size = pow(self.col_row, 2)
        return self.manhattanDistance==0
    ...
    def actions2(self):
        ...
        co=self.zeroindex
        self.moveToRight(co[0],co[1],moves)
        self.moveToLeft(co[0],co[1],moves)
        self.moveToUP(co[0],co[1],moves)
        self.moveToDown(co[0],co[1],moves)
        return moves
    ...
    def copy(self,to):
        ...
        return Puzzel(copyArray,zeroindex=to)
